...
const Index = ({
  ...
  return (
    <motion.div
      ...
      className="w-full max-w-4xl mx-auto fonts backdrop-blur-md border border-[#494949] rounded-[8px] px-4 md:px-8 py-8 cursor-pointer"
      style={{ backgroundColor: "color-mix(in oklab, #0f0f0f 25%, transparent)" }}
      onClick={() => setOpen((s) => !s)}
    >
      {/* ================= HEADER ================= */}
      <div className="py-6">
        {/* ========== MOBILE: number + chevron (row) ========== */}
        <div className="flex items-center justify-between md:hidden w-full">
          <motion.h3
            initial={false}
            animate={isHovered ? "hovered" : isVisible ? "visible" : "hidden"}
            variants={numberVariant}
            className="text-[#c2c0c0da] fonts text-[15px]"
        ...
        <div className="mt-4 md:hidden">
          <h1 className="text-3xl font-bold poppins text-white tracking-tighter">{title}</h1>
        ...
        <div className="hidden md:flex items-start justify-between w-full">
          <div className="flex items-start gap-4">
            ...
            {/* TITLE + SUBTITLE (only this shifts on hover) */}
            <div
              className="space-y-2"
              style={{
                transition: "all 0.4s",
      ...
      <AnimatePresence initial={false}>
        {open && (
          ...
          >
            ...
            <div className="mt-8 w-2/3 text-center md:w-full justify-center flex md:flex-row flex-col gap-4 mx-auto">
              <a
                href={github}
                className="px-4 py-3 text-sm rounded-md bg-[#101010e1] border border-[#494949] text-white hover:border-[#E1B84F] hover:bg-[#e1b84f5e] transition"
              ...
              <a
                href={documentation}
                className="px-4 py-3 text-sm rounded-md bg-[#101010e1] border border-[#494949] hover:border-[#E1B84F] hover:bg-[#e1b84f5e] text-white transition"
                onClick={(e) => e.stopPropagation()}
              >
                documentation ↗
              </a>
